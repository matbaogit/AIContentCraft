...
- July 30, 2025: Debugging Facebook content publishing issue - investigating why Facebook posts are receiving HTML extracted content instead of platform-specific generated content. Problem identified: webhook may not be returning expected array format with platform-specific content. Added comprehensive debugging logs to both frontend content selection logic and server webhook response parsing. Frontend shows "Không có nội dung" in UI but server logs show HTML content being sent to Facebook API, indicating content selection logic is falling back to extractedContent instead of using generatedContent from webhook response.
