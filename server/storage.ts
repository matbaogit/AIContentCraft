...
class DatabaseStorage implements IStorage {
  ...
  async getAnalyticsOverview(startDate: Date, endDate: Date): Promise<{
    ...
    try {
      ...
      // SEO content created (articles with SEO optimization)
      const seoContentQuery = `
        SELECT COUNT(*) as total
        FROM articles 
        WHERE created_at >= $1 AND created_at <= $2
          AND (meta_description IS NOT NULL OR keywords IS NOT NULL)
      `;

      // Images generated by AI (assuming AI-generated images have certain characteristics)
      const imagesGeneratedQuery = `
        SELECT COUNT(*) as total
        FROM images 
        WHERE created_at >= $1 AND created_at <= $2
      ...
      // Social content created (posts from social content generation)
      ...
      // Images created in period (all images)
      const imagesQuery = `
        SELECT COUNT(*) as total
        FROM images 
        WHERE created_at >= $1 AND created_at <= $2
      ...
      const [
        ...
        pool.query(socialContentQuery, [startDate, endDate]),
        pool.query(wordpressPostsQuery, [startDate, endDate]),
      ...
      return {
        ...
        imagesGenerated: parseInt(imagesGeneratedResult.rows[0]?.total || '0'),
        socialContentCreated: parseInt(socialContentResult.rows[0]?.total || '0'),
        wordpressPostsPublished: parseInt(wordpressPostsResult.rows[0]?.total || '0'),
