...
export function registerAdminRoutes(app: Express) {
  ...
  app.get('/api/admin/footer/sections', isAuthenticated, isAdmin, async (req, res) => {
    try {
      ...
      res.status(500).json({ success: false, error: 'Failed to fetch footer sections' });
    }
  });

  app.post('/api/admin/footer/sections', isAuthenticated, isAdmin, async (req, res) => {
    try {
      const { title, sectionKey, isActive, order } = req.body;
      const [section] = await db.insert(schema.footerSections).values({
        title,
        sectionKey,
        isActive,
        order
      ...
      res.status(500).json({ success: false, error: 'Failed to create footer section' });
    }
  });

  app.put('/api/admin/footer/sections/:id', isAuthenticated, isAdmin, async (req, res) => {
    try {
      const { id } = req.params;
      const { title, sectionKey, isActive, order } = req.body;
      const [section] = await db.update(schema.footerSections)
        .set({ title, sectionKey, isActive, order, updatedAt: new Date() })
        .where(eq(schema.footerSections.id, parseInt(id)))
        .returning();
      res.json({ success: true, data: section });
    } catch (error) {
      console.error('Error updating footer section:', error);
      res.status(500).json({ success: false, error: 'Failed to update footer section' });
    }
  });

  app.delete('/api/admin/footer/sections/:id', isAuthenticated, isAdmin, async (req, res) => {
    try {
      const { id } = req.params;
      await db.delete(schema.footerSections).where(eq(schema.footerSections.id, parseInt(id)));
      res.json({ success: true });
    } catch (error) {
      console.error('Error deleting footer section:', error);
      res.status(500).json({ success: false, error: 'Failed to delete footer section' });
    }
  });
  ...
  app.get('/api/admin/footer/links', isAuthenticated, isAdmin, async (req, res) => {
    try {
      const links = await db.query.footerLinks.findMany({
        orderBy: (footerLinks, { asc }) => [asc(footerLinks.sectionId), asc(footerLinks.order)]
      });
  ...
  app.post('/api/admin/footer/links', isAuthenticated, isAdmin, async (req, res) => {
    try {
      const { sectionId, label, href, isActive, order } = req.body;
      const [link] = await db.insert(schema.footerLinks).values({
        sectionId,
        label,
        href,
        isActive,
  ...
  app.put('/api/admin/footer/links/:id', isAuthenticated, isAdmin, async (req, res) => {
    try {
      const { id } = req.params;
      const { sectionId, label, href, isActive, order } = req.body;
      const [link] = await db.update(schema.footerLinks)
        .set({ sectionId, label, href, isActive, order, updatedAt: new Date() })
        .where(eq(schema.footerLinks.id, parseInt(id)))
        .returning();
      res.json({ success: true, data: link });
    } catch (error) {
  ...
  app.delete('/api/admin/footer/links/:id', isAuthenticated, isAdmin, async (req, res) => {
    try {
      const { id } = req.params;
      await db.delete(schema.footerLinks).where(eq(schema.footerLinks.id, parseInt(id)));
      res.json({ success: true });
    } catch (error) {
  ...
  app.get('/api/admin/footer/social', isAuthenticated, isAdmin, async (req, res) => {
    try {
      ...
      res.status(500).json({ success: false, error: 'Failed to fetch footer social links' });
    }
  });

  app.post('/api/admin/footer/social', isAuthenticated, isAdmin, async (req, res) => {
    try {
      const { platform, url, isActive, order } = req.body;
      const [socialLink] = await db.insert(schema.footerSocialLinks).values({
        platform,
        url,
        isActive,
        order
      ...
      res.status(500).json({ success: false, error: 'Failed to create footer social link' });
    }
  });

  app.put('/api/admin/footer/social/:id', isAuthenticated, isAdmin, async (req, res) => {
    try {
      const { id } = req.params;
      const { platform, url, isActive, order } = req.body;
      const [socialLink] = await db.update(schema.footerSocialLinks)
        .set({ platform, url, isActive, order, updatedAt: new Date() })
        .where(eq(schema.footerSocialLinks.id, parseInt(id)))
        .returning();
      res.json({ success: true, data: socialLink });
    } catch (error) {
      console.error('Error updating footer social link:', error);
      res.status(500).json({ success: false, error: 'Failed to update footer social link' });
    }
  });

  app.delete('/api/admin/footer/social/:id', isAuthenticated, isAdmin, async (req, res) => {
    try {
      const { id } = req.params;
      await db.delete(schema.footerSocialLinks).where(eq(schema.footerSocialLinks.id, parseInt(id)));
      res.json({ success: true });
    } catch (error) {
      console.error('Error deleting footer social link:', error);
      res.status(500).json({ success: false, error: 'Failed to delete footer social link' });
    }
  });

  // Footer Settings Routes
  app.get('/api/admin/footer/settings', isAuthenticated, isAdmin, async (req, res) => {
    try {
      const settings = await db.query.footerSettings.findFirst();
      res.json({ success: true, data: settings });
    } catch (error) {
      console.error('Error fetching footer settings:', error);
      res.status(500).json({ success: false, error: 'Failed to fetch footer settings' });
    }
  });

  app.put('/api/admin/footer/settings', isAuthenticated, isAdmin, async (req, res) => {
    try {
      ...
      let settings;
      if (existing) {
        [settings] = await db.update(schema.footerSettings)
          .set({ description, copyrightText, updatedAt: new Date() })
          .where(eq(schema.footerSettings.id, existing.id))
          .returning();
      } else {
        [settings] = await db.insert(schema.footerSettings)
          .values({ description, copyrightText })
          .returning();
      }
      ...
      res.status(500).json({ success: false, error: 'Failed to update footer settings' });
    }
  });
}