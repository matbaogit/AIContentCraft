...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ========== Debug Legal Pages API ==========
  // Debug endpoint to see all legal pages data
  app.get('/api/debug/legal-pages', async (req, res) => {
    try {
      const pages = await db.select().from(schema.legalPages);
      ...
      res.json({
        ...
        data: pages.map(page => ({
          ...
          path: page.path,
          content_vi_length: page.content_vi?.length || 0,
          content_en_length: page.content_en?.length || 0,
          content_vi_preview: page.content_vi?.substring(0, 200) + '...',
          content_en_preview: page.content_en?.substring(0, 200) + '...'
        }))
      });
    } catch (error) {
      console.error('Error in debug legal pages:', error);
      res.status(500).json({
        success: false,
        error: 'Debug failed',
  ...
  // ========== Fix Legal Pages Content API ==========
  // Temporary endpoint to fix legal pages with unique content
  app.post('/api/debug/fix-legal-content', async (req, res) => {
    try {
      console.log('🔧 Fixing legal pages content...');
...
<p><strong>Contact:</strong> support@seoaiwriter.com</p>`,
        ...
        .where(eq(schema.legalPages.id, 'privacy-policy'));
...
<p><strong>Support contact:</strong> support@seoaiwriter.com</p>`,
        ...
        .where(eq(schema.legalPages.id, 'data-deletion'));
...
<p><strong>Contact:</strong> support@seoaiwriter.com</p>`,
        ...
        .where(eq(schema.legalPages.id, 'terms-of-service'));

      console.log('✅ Legal pages content fixed successfully!');
      
      res.json({
        success: true,
        message: 'Legal pages updated with unique content',
        updatedPages: ['privacy-policy', 'data-deletion', 'terms-of-service']
      });
      
    } catch (error) {
      console.error('❌ Error fixing legal pages content:', error);
      res.status(500).json({
        success: false,
        error: 'Failed to fix legal pages content',
        details: error.message
      });
    }
  });
